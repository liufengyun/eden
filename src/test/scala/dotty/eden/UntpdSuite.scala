...
class UntpdSuite extends EdenSuite {
  ...
  checkUntpd("a match { case a.name => ; case _: b.M => ; case `x` => }")
  checkUntpd("a match { case x::xs =>  }")
  ...
  // checkUntpd("a match { case M(a, x @ _*) => }")
  // checkUntpd("a match { case (_:_*) =>  }")
  checkUntpd("a match { case (_: A) =>  }")
  checkUntpd("a match { case Some[Int](x) => }")
  checkUntpd("a match { case x: Option[Int] => }")
  checkUntpd("a match { case x: Option[_] => }")
  checkUntpd("a match { case x: A#B => }")
  checkUntpd("a match { case x: c.M => }")
  checkUntpd("a match { case x: c.type => }")
